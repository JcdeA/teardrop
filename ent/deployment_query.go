...
func (dq *DeploymentQuery) QueryDomains() *DomainQuery {
	query := &DomainQuery{config: dq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := dq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (dq *DeploymentQuery) WithDomains(opts ...func(*DomainQuery)) *DeploymentQuery {
	query := &DomainQuery{config: dq.config}
	for _, opt := range opts {
		opt(query)
	}
	dq.withDomains = query
	return dq
}
...
//		GroupBy(deployment.FieldBranch).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
func (dq *DeploymentQuery) sqlAll(ctx context.Context) ([]*Deployment, error) {
	...
	if query := dq.withDomains; query != nil {
		...
		query.Where(predicate.Domain(func(s *sql.Selector) {
			s.Where(sql.InValues(deployment.DomainsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.deployment_domains
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "deployment_domains" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "deployment_domains" returned %v for node %v`, *fk, n.ID)
			}
