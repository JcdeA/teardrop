...
func (dq *DomainQuery) QueryDeployment() *DeploymentQuery {
	query := &DeploymentQuery{config: dq.config}
...
func (dq *DomainQuery) WithDeployment(opts ...func(*DeploymentQuery)) *DomainQuery {
	query := &DeploymentQuery{config: dq.config}
	for _, opt := range opts {
		opt(query)
	}
	dq.withDeployment = query
	return dq
}
...
func (dq *DomainQuery) sqlAll(ctx context.Context) ([]*Domain, error) {
	...
	if query := dq.withDeployment; query != nil {
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "deployment_domains" returned %v`, n.ID)
			}
			for i := range nodes {
